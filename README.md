...
## v0.12: Precise cuts, checked connections and steadier exports

Nick selected improvements 1, 2 and 5. These changes run behind Create and report outcomes in the existing Review edits panel:

1. **Search actual source frames.** Coarse inspection, dense overlap verification, quiet-pause candidates and fine cut searches use the source's presentation timestamps. This includes 24/25/30/50/60, fractional rates and variable frame rates. A 60 fps clip contributes its intervening frames instead of being searched only on a 30 fps grid. Outgoing motion prediction uses the actual interval to the exclusive cut boundary. Original clip ends remain valid; verified overlap seams retain their shared timeline relationship. Packet timing is read once for the Create job, reused for rendering and released from the UI plan afterward.
2. **Check the rendered enhancement.** After encoding, decode a short sequence around every interpolated or framing/color-corrected connection. Compare it with the original source sequence for measured motion jumps, abrupt brightness/color changes, tracking disagreement and local detail loss. The check includes the transition back to unchanged footage. It uses bounded 192-pixel previews alongside the existing output-resolution admission checks. If an enhancement fails or cannot be checked, discard that export and render once with original pictures at the same chosen cuts. This fallback disables optional effects throughout that export, preserves order, trims, duration and source sound, and is disclosed in Review edits.
3. **Keep long exports bounded.** Resample and encode audio in one-second blocks, retaining absolute sample positions, stereo/mono behavior, leading silence and the same five-millisecond ramps at clip boundaries. Release encoding resources before decoding the finished movie. On a qualifying encoder/resource failure, release the failed attempt, check a smaller profile with at most 30 fps and a reduced bitrate, and retry once. Source-read/decryption errors and cancellation do not trigger this retry. A second failure stops with the selected clips still available.

The temporal checker is a conservative measurement, not a guarantee that every artifact or subjective editing problem can be detected. It may choose an ordinary cut when the benefit is uncertain. A rejected enhancement requires another rendering pass; a runtime recovery can also reduce dimensions/cadence. There is no new model, paid service, remote processing or normal-path control. The existing three-minute/twelve-clip and export-size bounds still apply. The finished Blob remains in memory only; there is no previous-upload archive.

## Included v0.11: Picture quality and cleaner connections
...
This update applies to the confirmed v0.11 installation. Extract `Cutroom_v0.12_GitHub_Update.zip` and upload all **23 files** to the top level of `MagnificoDesign/Cutroom` on `main`, replacing matching files and adding the new modules/tests. Keep the other existing files. Do not upload just the ZIP. The hash manifest describes the complete 51-file release after applying this update. No build command, personal videos or Node dependencies are needed. Keep GitHub Pages on main/root.

After Pages deploys, open [Cutroom v0.12](https://magnificodesign.github.io/Cutroom/?release=0.12) online to download the update, close all Cutroom Safari tabs and its Home Screen instance, then reopen. Confirm **v0.12**. Changed modules use v12 URLs and a new service-worker cache that includes every new module for offline use. The service worker waits for old instances to close so it cannot replace code during an import. **Do not clear website data to update**: that would erase the current project and password. Previous imports are still not archived; Create New Video deletes the imported project copies.
