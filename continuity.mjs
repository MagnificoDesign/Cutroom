...
import { checkBank, minimumPiece } from './edit-policy.mjs?v=14';
import { sourceTimes, coarseFrames, nearbyFrames } from './cut-timing.mjs?v=14';
import { compactFrames, proposeConnections, selectSequence, sourceEnd } from './continuity-core.mjs?v=14';
import { refineMotion } from './motion.mjs?v=14';
import { boundaryCost, signature, validatePlan } from './planner.mjs?v=14';
import { checkConnection, checkSimilarConnection } from './connection.mjs?v=14';
import { pictures } from './transitions.mjs?v=14';
import { validateBridgeImages, validateMatchImages } from './transition-core.mjs?v=14';
import { validateFinishing } from './finish-core.mjs?v=14';
...
export async function analyzeContinuity({ clips: originals, getBlob, signal, onProgress = () => {}, inspect = inspectMedia, readSources = inspectSources, readPictures = pictures }) {
  ...
  const refine = async edge => {
    ...
    for (const end of ends) for (const start of starts) {
      ...
      if (score.similarity >= .87 && score.mismatch <= .28 && score.continuation <= .32) candidates.push({ end, start, cost: score.cost });
    }
    ...
    for (const candidate of diverse) {
      ...
      if (!match) { remember(key, null); continue; }
      ...
      const verified = ok ? { cost: match.cost, status: 'verified', kind: 'continuity', similar: !!match.similar,
  ...
  return { ...plan, continuity: true, improved: true, reviewed: [], failures, checkedConnections: checked, candidateCount, analyzedCount: clips.length,
