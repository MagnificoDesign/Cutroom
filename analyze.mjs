...
export function frameTimes(start, end, duration, rate = 30) {
  const out = [];
  for (let i = Math.max(0, Math.floor(start * rate)); i / rate < Math.min(duration - .0001, end); i++) out.push(i / rate);
  return out;
}

export async function analyzeJoins({ clips, getBlob, signal, onProgress = () => {}, inspect = inspectMedia }) {
  ...
  for (let index = 0; index < selected.length; index++) {
    ...
    let aa, bb;
    try {
      aa = await inspect(await getBlob(a, signal), frameTimes(lo, a.duration, a.duration, 60), signal, { audioRange: [lo, a.duration] });
      check(signal);
      bb = await inspect(await getBlob(b, signal), frameTimes(0, hi, b.duration, 60), signal, { audioRange: [0, hi] });
      check(signal);
  ...
  for (let i = 0; i < first.segments.length; i++) {
    ...
    const fineIn = i ? frameTimes(Math.max(0, part.start - .2), Math.min(clip.duration - keep, part.start + .2) + .001, clip.duration) : [];
    const fineOut = i + 1 < first.segments.length ? frameTimes(Math.max(keep, part.end - .2), Math.min(clip.duration, part.end + .2), clip.duration) : [];
    const times = new Set();
